...
sc2_epiym <- ddply(filt, .(filt$epiym, filt$greek), nrow, .drop = FALSE)
names(sc2_epiym) <- c("epiym", "variants", "n")
sc2_epiym <- ddply(sc2_epiym, .(epiym), transform, percentage = n/sum(n))

ggp <- ggplot(sc2_epiym, aes(x = epiym, y = percentage)) +
...
save_plot("SC2ProportionalStackedArea_Output.svg", ggp, base_height = 3, base_width = 6)