...
p_load(areaplot, cowplot, dplyr, ggplot2, lubridate, plyr, readr, this.path, tidyr)

setwd(dirname(this.path()))

# https://www.epicov.org/epi3/frontend
# EpiCov > Search > Download > Input for the Augur pipeline
input <- list.files(pattern = "*.tar", full.names = TRUE)
ldply(.data = input, .fun = untar)
...
gisaid_filt$epiym <- ifelse(
  grepl("^\\d{4}-\\d{2}-\\d{2}$", gisaid_filt$date),
  substr(gisaid_filt$date, 1, 7),
  gisaid_filt$date
)
...
save_plot(sub("\\.tar$", ".pdf", input), sc2_epiym_plot, base_height = 3, base_width = 6)
save_plot(sub("\\.tar$", ".png", input), sc2_epiym_plot, base_height = 3, base_width = 6)