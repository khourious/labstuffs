...
countdata <- read.table("counts_DENV_CTRL.txt", header=TRUE, row.names=1)
...
# plot dispersions
pdf("qc-dispersions_DENV_CTRL.pdf", 50, 50, pointsize=100)
...
pdf("qc-distance-heatmap_DENV_CTRL.pdf", w=50, h=50, pointsize=100)
...
pdf("qc-pca_DENV_CTRL.pdf", 50, 50, pointsize=70)
rld_pca(rld, colors=mycols, intgroup="condition", xlim=c(-35, 45))
dev.off()

# decrease the fold change noise with shrinkage function 
resShrink <- lfcShrink(dds,contrast=c("condition","DENV","CTRL"),type="ashr")

# MA plot of the effect of the shrinkage correction
res <- results(dds)
mcols(res,use.names=TRUE)
pdf("qc-shrinkage-correction_DENV_CTRL.pdf", 50, 50, pointsize=80)
par(mfrow=c(1,2))
...
pdf("diffexprShrinkage-volcanoplot_DENV_CTRL.pdf", 10, 10, pointsize=20)
...
# write results
write.csv(resShrinkdata, file="diffexprShrinkage-results_DENV_CTRL.csv")